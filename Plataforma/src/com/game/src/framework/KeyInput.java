...
public class KeyInput extends KeyAdapter 
{
	...
	public void keyPressed(KeyEvent e) 
	{
		...
		for (int i = 0; i < handler.getObjects().size(); i++) 
			...
			{
				if(key == KeyEvent.VK_RIGHT) handler.getObjects().get(i).setVelX(5);
				if(key == KeyEvent.VK_LEFT) handler.getObjects().get(i).setVelX(-5);
	...
	public void keyReleased(KeyEvent e) 
	{
		...
		for (int i = 0; i < handler.getObjects().size(); i++) 
			if(handler.getObjects().get(i).getId().equals(ObjectId.Player)) 
			{
				if(key == KeyEvent.VK_RIGHT) handler.getObjects().get(i).setVelX(0);
				if(key == KeyEvent.VK_LEFT) handler.getObjects().get(i).setVelX(0);
